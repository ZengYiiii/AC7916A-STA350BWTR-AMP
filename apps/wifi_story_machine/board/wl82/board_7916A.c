...
#endif
   .tx_pin = TCFG_DEBUG_TX_PORT,
   .rx_pin = TCFG_DEBUG_RX_PORT,
   .max_continue_recv_cnt = 1024,
   .idle_sys_clk_cnt = 500000,
   .clk_src = PLL_48M,
   .flags = UART_DEBUG,
...
/*
lIS_IO      MCLK    SCLK    LRCK    CH0     CH1     CH2     CH3
IIS_PORTA   PA4     PA5     PA6     PA7     PA8     PA9     PA10
IIS_PORTC   PCO     PC1     PC2     PC3     PC4     PC5     PC6
...
static const struct iis_platform_data iis0_data = {
    ...
    .mclk_output = 1,
    .slave_mode = 0,
    .keep_alive = 1, // 保持IIS0硬件常开,使MCLK常驻,供外置STA350BW功放锁定PLL
    .dump_points_num = 320,
};

/*
 * 主动点亮IIS0的MCLK/BCLK/LRCK,供外置STA350BW功放(I2S从机)锁定内部PLL。
 * 外置功放的PLL与I2C控制都依赖MCLK,而SDK的IIS0时钟默认按需开启(只在
 * audio_dec打开"iis0"解码流时点亮),导致开机阶段功放探测/初始化时无时钟。
 * 配合iis0_data的keep_alive=1,开机即让时钟常驻,功放可在首次播放前完成初始化。
 */
void board_iis0_clk_keep_on(void)
{
    iis_set_sample_rate(44100, 0);
    iis_open((struct iis_platform_data *)&iis0_data, 0);
}
