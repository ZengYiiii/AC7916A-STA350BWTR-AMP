 ...
 */
/* ----------------------------------------------------------------------------*/
__attribute__((weak)) void BLE_RX_Event(uint16_t att_handle, uint16_t transaction_mode, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    (void)(att_handle);
...
static int att_write_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t transaction_mode, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    BLE_RX_Event(att_handle, transaction_mode, offset, buffer, buffer_size);
    return 0;
}
...
/* 供 EQ 调音模块经 ae3c 通知回传数据(如 QUERY 响应),供 user_app.c 调用 */
int eq_notify_send(const u8 *data, u16 len)
{
    return app_send_user_data(ATT_CHARACTERISTIC_ae3c_01_VALUE_HANDLE, data, len, ATT_OP_AUTO_READ_CCC);
}
